...
class Smtpmails extends Model{
    ...
    public static function smtpmail($mail_to, $subject, $message, $headers = '') {
        ...
        //Настройки почты
        $config['smtp_username'] = 'info@butterfly-design.ru';  //Смените на имя своего почтового ящика из ISPManager.
        $config['smtp_password'] = 'qwe123';  //Измените пароль.
        $config['smtp_from'] = 'butterfly-design.ru'; //Ваше имя - или имя Вашего сайта. Будет показывать при прочтении в поле "От кого".
        ...
        if (!static::server_parse($socket, "220", __LINE__))
            return false;

        fputs($socket, "EHLO " . $config['smtp_host'] . "\r\n");
        if (!static::server_parse($socket, "250", __LINE__)) {
            if ($config['smtp_debug'])
        ...
        if (!static::server_parse($socket, "334", __LINE__)) {
            if ($config['smtp_debug'])
        ...
        if (!static::server_parse($socket, "235", __LINE__)) {
            if ($config['smtp_debug'])
        ...
        if (!static::server_parse($socket, "354", __LINE__)) {
            if ($config['smtp_debug'])
    ...
    public static function server_parse($socket, $response, $line = __LINE__) {
        ...
        while (substr($server_response, 3, 1) != ' ') {
            if (!($server_response = fgets($socket, 256))) {
                if ($config['smtp_debug'])
                    echo "<p>Проблемы с отправкой почты!</p>$response $line";
                return false;
            }
        }
...
//Отправить почту сразу нескольким получателям, ящики получателей пишем через запятую.
    ...
    public static function smtpmassmail($mail_to, $subject, $message, $headers = '') {
        $mailaddresses = explode(",", $mail_to);
        foreach ($mailaddresses as $mailaddress)
            static::smtpmail($mailaddress, $subject, $message, $headers);
    }
