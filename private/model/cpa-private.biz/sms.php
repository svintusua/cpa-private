...
class Sms{
	protected static $login = 'roma.conect';	
	protected static $password = 'CgbhfktxrfVjz3455';	

	
	public static function getSmsTextByUserId($partner_id){
		if(isset($partner_id) && !empty($partner_id) && filter_var($partner_id, FILTER_VALIDATE_INT)){
			$q = 'SELECT * FROM `sms_message` WHERE partner_id='.$partner_id.' LIMIT 7';
			return IOMysqli::table($q);
		}else{
			return false;
		}
	}
	public static function getSmsTextByUserIdAndType($partner_id, $type){
		if(isset($partner_id) && !empty($partner_id) && filter_var($partner_id, FILTER_VALIDATE_INT) && isset($type) && !empty($type) && filter_var($type, FILTER_VALIDATE_INT)){
			$q = 'SELECT message FROM `sms_message` WHERE partner_id='.$partner_id.' and type='.$type.' LIMIT 1';
			return IOMysqli::one($q);
		}else{
			return false;
		}
	}
	public static function setSmsText($partner_id, $type, $message){

		if(isset($message) && !empty($message) && isset($partner_id) && !empty($partner_id) && filter_var($partner_id, FILTER_VALIDATE_INT) && isset($type) && !empty($type) && filter_var($type, FILTER_VALIDATE_INT)){
			$q = 'SELECT id FROM `sms_message` WHERE partner_id='.$partner_id.' and type='.$type.' LIMIT 1';
			$prov = IOMysqli::one($q);
			
			if($prov){
				$q = 'UPDATE `sms_message` SET `message`=\''.$message.'\' WHERE id='.$prov.' and type='.$type.' and partner_id='.$partner_id.' LIMIT 1';
			}else{
				$q = 'INSERT INTO `sms_message`(`partner_id`, `message`, `type`) VALUES ('.$partner_id.', \''.$message.'\','.$type.')';
			}
	...
	public static function sendOneSms($phones,$message){
		if(isset($phones) && !empty($phones) && isset($message) && !empty($message)){
			return file_get_contents('http://smsc.ru/sys/send.php?login='.static::$login.'&psw='.static::$password.'&phones='.urlencode($phones).'&mes='.urlencode($message).'&translit=1');
		}else{
			return false;
		}
	}
	...
	public static function sendSms($partner_id, $type, $phones){
		if(isset($phones) && !empty($phones) && isset($partner_id) && !empty($partner_id) && filter_var($partner_id, FILTER_VALIDATE_INT) && isset($type) && !empty($type) && filter_var($type, FILTER_VALIDATE_INT)){
			$q = 'SELECT message FROM `sms_message` WHERE partner_id='.$partner_id.' and type='.$type.' LIMIT 1';
			$message = IOMysqli::one($q);
			if(!$message){
				switch($type){
					case 1:
			...
			return file_get_contents('http://smsc.ru/sys/send.php?login='.static::$login.'&psw='.static::$password.'&phones='.urlencode($phone).'&mes='.urlencode($message));
		}else{
			return false;
		}
	}
...
public static function send_sms($phones, $message, $translit = 0, $time = 0, $id = 0, $format = 0, $sender = false, $query = "", $files = array())
{
	...
	$m = static::_smsc_send_cmd("send", "cost=3&phones=".urlencode($phones)."&mes=".urlencode($message).
		...
		if ($m[1] > 0){
			...
			if($m[3]<=50){
				$msg = '<html>
					<head>
					<meta charset="urf-8"/>
					<title>Низкий баланс СМС сервиса</title>
					</head>
						<body>
							<p>Низкий баланс на SMSC.RU</p>
							<p>баланс на данный момент: '.$m[3].'</p>
						</div>
					</body>
					</html>';
            ...
            $headers .= "From: Приватная партнерская программа <no-reply@cpa-private.biz>\r\n"; 
				mail('sl.lok@yandex.ru', "Низкий баланс на SMSC.RU", $msg, $headers);
			}
			// return json_encode(array('response'=>'success','text'=>"Сообщение отправленно. (стоимость: ".$m[2].", баланс: ".$m[3].")"));
			return json_encode(array('response'=>'success','text'=>"Сообщение отправленно"));
		}else{ 
			// echo "Ошибка №", -$m[1], $m[0] ? ", ID: ".$m[0] : "", "\n";
			$id = $m[0] ? ", ID: ".$m[0] : "";
			$text = "Ошибка № ".-$m[1].$id ;
			return json_encode(array('response'=>'error','text'=>$text));
		}
	// }
...
// {
	...
	// $m = static::_smsc_send_cmd("send", "cost=1&phones=".urlencode($phones)."&mes=".urlencode($message).
					// ($sender === false ? "" : "&sender=".urlencode($sender)).
...
// {
	// $m = static::_smsc_send_cmd("status", "phone=".urlencode($phone)."&id=".urlencode($id)."&all=".(int)$all);
...
// {
	// $m = static::_smsc_send_cmd("balance"); // (balance) или (0, -error)
...
public static function _smsc_send_cmd($cmd, $arg = "", $files = array())
{
	...
	// $url = (SMSC_HTTPS ? "https" : "http")."://smsc.ru/sys/$cmd.php?login=".SMSC_LOGIN."&psw=".SMSC_PASSWORD."&fmt=1&charset=".SMSC_CHARSET."&".$arg;
	...
	do {
		...
		$ret = static::_smsc_read_url($url, $files);
	}
	while ($ret == "" && ++$i < 4);
